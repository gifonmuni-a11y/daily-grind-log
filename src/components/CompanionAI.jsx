...
const AVATAR_LINKS = {
  ...
  ketuk: 'https://eekeixvvrspyguawqmnl.supabase.co/storage/v1/object/public/Public/entry-images/ketuk.gif'
}
...
export default function CompanionAI({ userStats, profile, onClose }) {
  ...
  const highlightPlainWords = (str, lineIdx, partIdx) => {
    if (!str) return str
    const greetingsPattern = "(?:Selamat pagi|Selamat siang|Selamat sore|Selamat malam|Selamat beraktivitas|Bangun dan waktunya bersinar)"
    const namePattern = userName && userName !== 'Trainer' ? `\\b${userName}\\b` : ""
    const numberPattern = "\\b\\d+\\b"
    
    const patterns = [greetingsPattern, namePattern, numberPattern].filter(Boolean).join("|")
  ...
  const handleAvatarTap = () => {
    ...
    setAvatarState('ketuk')
    if (isMuted) {
      setTimeout(() => setAvatarState('diam'), 2000)
    } else {
      speakText(reply, 'diam', 'ketuk')
    }
  }
