...
const MASTER_34_CATEGORIES = [
  ...
  { name: 'Boxing / Combat', tokoh_terkenal: 'Mike Tyson: Semua orang punya rencana sampai sebuah pukulan telak mendarat di mulut mereka.', apa_itu: 'Latihan seni bela diri berbasis pukulan pukulan presisi dan pergerakan kaki taktis untuk pengkondisian fisik.', manfaatnya: 'Melatih refleks instan sistem saraf, ketangkasan gerak kaki (footwork), koordinasi mata-tangan, serta membakar kalori dalam jumlah masif.', tata_cara_atau_gerakan: 'Lakukan kombinasi pukulan fundamental: Jab, Cross, Hook, dan Upper-cut dipadukan dengan gerakan menghindar (weaving) di depan samsak atau cermin (shadow boxing).', id_video: '3u6lojo40a8' },
...
export default function CompanionAI({ userStats, onClose }) {
  ...
  useEffect(() => {
    ...
    setMessages([{ sender: 'seolha', text: `${greetingText}, Trainer. Ada yang bisa saya bantu untuk menemani sesi latihan harian Anda?`, media: null }])
    fetchDailyLimit()
  ...
  const handleSend = async (e, customMsg = null, isFaq = false, isAllCategories = false) => {
    ...
    if (!isFaq && dailyCount >= 5) {
      setMessages(prev => [...prev, { sender: 'user', text: msgToSend }, { sender: 'seolha', text: 'Energi aku sudah habis untuk hari ini (Batas 5 pertanyaan telah tercapai). Kita obrol lagi besok ya!', media: null }])
    ...
    if (isAllCategories) {
      setTimeout(() => {
        setMessages(prev => [...prev, { sender: 'seolha', text: "Berikut adalah daftar matrix **34 KATEGORI LATIHAN LENGKAP**. Silakan klik tiap kategori untuk memuat video panduan dan penjelasannya:", media: null, multiMedia: MASTER_34_CATEGORIES }])
        setLoading(false)
      }, 600)
      return
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#000000] p-4 max-w-lg mx-auto select-none">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-[#211D2C]">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 font-mono text-[11px] font-bold text-text-high bg-[#100E16] border border-[#7C5CFF]/30 px-2 py-1 rounded shadow-[0_0_10px_rgba(124,92,255,0.05)]">
            <svg className="w-3.5 h-3.5 text-[#7C5CFF] fill-current" viewBox="0 0 24 24"><path d="M16 6H4c-1.1 0-2 .9-2 2v8c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm4 4h2v4h-2v-4z"/></svg>
      ...
      <div className="main-chat-container flex-1 overflow-y-auto py-3 space-y-4 pr-1">
        ...
        {loading && <div className="flex justify-start"><div className="bg-[#100E16] border border-[#211D2C] p-3 rounded-xl flex items-center gap-2 font-mono text-xs text-text-dim"><Loader2 size={12} className="animate-spin text-accent" />Seolha sedang berpikir...</div></div>}
