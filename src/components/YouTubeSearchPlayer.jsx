...
import { Search, X, AlertTriangle, Play, SkipForward, SkipBack, ListMusic, Trash2, MonitorPlay, Library, Upload, Timer, Share2, Disc3, Mic2 } from 'lucide-react'
// html2canvas DIHAPUS dari import atas biar nggak bikin layar blank saat loading!

// Bersihkan HTML entity dengan aman
...
// Inisialisasi Database Lokal dengan Proteksi Crash
const initDB = () => {
  return new Promise((resolve, reject) => {
    ...
    try {
      const request = window.indexedDB.open('HoarderMusicDB', 1)
...
export default function YouTubeSearchPlayer() {
  ...
  const loadLocalSongs = async () => {
    try {
      ...
      request.onsuccess = () => {
        ...
        setLocalSongs(data.reverse())
      }
    } catch (err) {
      console.error('Gagal akses memori lokal:', err)
      setLocalSongs([]) // Fallback aman
    }
  }
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      setError('Gagal menyimpan lagu ke HP.')
    }
    e.target.value = '' 
  }
  ...
  // --- INIT YOUTUBE PLAYER (Anti Race Condition) ---
  useEffect(() => {
    let cancelled = false
        ...
        events: { 
          onReady: () => { 
            if (pendingTrackRef.current) {
              playerRef.current.loadVideoById(pendingTrackRef.current.videoId)
    ...
    }).catch(() => console.error("Gagal load YouTube API"))
  ...
  // --- SHARE KE SOSMED (Dynamic Import Anti-Crash) ---
  const handleShare = async () => {
    ...
    try {
      // Import secara dinamis agar tidak memberatkan loading awal aplikasi!
  ...
  return (
    <div className="relative">
      ...
      <div className="overflow-hidden absolute top-[-9999px] left-[-9999px]">
        {nowPlaying && (
          <div ref={shareCardRef} className="w-[400px] h-[700px] bg-gradient-to-b from-[#1a1433] to-[#050508] p-8 flex flex-col items-center justify-center relative shadow-2xl">
            <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[300px] h-[300px] bg-[#7C5CFF] rounded-full blur-[120px] opacity-20" />
            <img src={nowPlaying.thumb} alt="cover" className="w-64 h-64 object-cover rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] z-10 border border-[#211D2C]" crossOrigin="anonymous" />
            <div className="z-10 mt-10 text-center w-full">
              ...
              <h1 className="text-2xl font-bold text-white mb-2 leading-snug line-clamp-2">{decodeHtmlEntities(nowPlaying.title)}</h1>
              <p className="text-[#a09bb3] text-lg font-mono">{nowPlaying.type === 'local' ? 'Local Storage' : decodeHtmlEntities(nowPlaying.channel)}</p>
            </div>
            ...
            <p className="absolute bottom-6 font-mono text-[10px] text-gray-600 uppercase tracking-widest">Shared via VibePlayer</p>
          </div>
        )}
      </div>
      ...
      {!showQueue && (
        <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto mb-3">
          ...
          {(activeTab === 'local' ? (localSongs || []) : (results || [])).map(track => {
            const trackId = track?.id || track?.videoId || Math.random()
            return (
      ...
      {showQueue && (
        <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto mb-3">
          {(!queue || queue.length === 0) && <p className="font-mono text-[10px] text-gray-500 text-center py-4">Antrean kosong</p>}
          {(queue || []).map((track, idx) => {
