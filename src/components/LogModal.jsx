...
export default function LogModal({ userId, maxDayNumber, editEntry, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-4 select-none animate-in fade-in duration-150">
      ...
      {/* 🎯 CONTAINER MODAL UTAMA: DIUBAH MENJADI PURE KOTAK LANCIP (rounded-none) */}
      <div className="w-full max-w-md bg-[#100E16] border border-[#211D2C] p-6 rounded-none flex flex-col gap-5 max-h-[88vh] overflow-y-auto relative border-box [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        
        {/* 🎯 SIKU SYSTEM UNGU DI 4 SUDUT LUAR KOTAK CONTAINER UTAMA (NEMPEL PAS DI TEPIAN BORDER LUAR) */}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 font-mono text-xs text-[#EDEAF6] pb-6">
          ...
          {/* 🎯 CUSTOM OVERLAY KALENDER RPG SYSTEM: DIUBAH MENJADI PURE KOTAK LANCIP (rounded-none) */}
          {showDatePicker && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in zoom-in-95 duration-100">
              <div className="bg-[#100E16] border border-[#211D2C] w-full max-w-sm rounded-none p-4 flex flex-col gap-3 relative shadow-2xl">
                ...
                {/* 🎯 SIKU UNGU MODAL KALENDER: SEKARANG NEMPEL PAS DI TEPIAN KOTAK LUAR */}
                ...
                {/* TAMPILAN GRID EDIT TAHUN RAPID */}
